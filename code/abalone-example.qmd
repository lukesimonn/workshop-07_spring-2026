...
H~0~: **pH has no effect on abalone growth.**  
...
abalone_clean <- abalone |>   
  ...
  # renaming columns to make more standard
...
# base layer: ggplot
ggplot(data = abalone_clean, # data from clean data frame
       aes(x = mean_ph, # x axis: pH
           y = change_in_area)) + # y-axis: change in area
...
**Missing value comes from one pair of tanks where pH sensor malfunctioned.**  
...
`x`, `predicted`, `std.error`, `conf.low`, `conf.high` (not worrying about `group` for now)
...
**At a pH of 8 the model predicts change in abalone shell surface area to be 5.36 [95% CI: 5.08,. 5.64] (mm^2^ d^-1^).**
